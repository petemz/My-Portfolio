			...
			<header className="fixed z-40 top-0 h-20 bg-inherit px-6 w-full flex justify-between items-center"> 
				...
				<div className="flex items-center">
					...
					<button 
						...
						className={`${isDarkMode ? 'border-[#eee]' : 'border-black' } flex w-7 h-7 -sm:ml-8 border-2 rounded-full overflow-hidden`}
					>
						<div className={`${isDarkMode ? 'bg-[#eee]' : 'bg-black'} h-full w-[43%]`}></div>
					</button>

					<button onClick={() => setOverlayMenu(true)} className="text-4xl hidden ml-5 sm:block">
						{<FiMenu/>}
					</button>
				</div>
